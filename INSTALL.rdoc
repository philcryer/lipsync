...
Now that you have a working 2 node setup, it's time to install on more clients. All you need to do is follow the same client install directions on each client you want to add. Be sure to use the same server IP or domainname, this way unison will work in a star style setup, with the server being the hub, and the clients all serving as spokes. This way the hub 'wins' if it has something the spokes need, but will sync-up by deleting any missing files from the spoke in the hub. It sounds more complicated that it is.
